...
const POINTER = { ndc: new THREE.Vector2(0, 0), world: new THREE.Vector3(), active: false, lastMove: performance.now(), activity: 0 };
...
window.addEventListener('scroll', () => { scrollTarget = getScrollTarget(); });
...
function updatePointer() {
  if (POINTER.active) {
    ...
    if (Math.abs(dir.z) > 1e-4 && t > 0 && isFinite(t)) {
