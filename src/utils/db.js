...
//
// Login, registro, perfil y compras usan Supabase (base de datos real en
// internet). El chat con IA (/api/chat) sigue usando el servidor Node
// (server/server.js) porque necesita esconder la llave secreta de Anthropic.

import { supabaseClient, NEXUS_SUPABASE_READY } from './supabaseClient.js';

function requireSupabase() {
    if (!NEXUS_SUPABASE_READY) {
        throw new Error(
...
export const createPurchase = async ({ userId, packageId, packageName, amount, paymentMethod }) => {
    const supabase = requireSupabase();
...
export const getCurrentUser = async () => {
    if (!NEXUS_SUPABASE_READY) return null;
...
export const logoutUser = async () => {
    if (!NEXUS_SUPABASE_READY) return;
...
// Los "leads" (dudas que la IA no resolvió) se guardan directo en Supabase.
export const sendLead = async ({ nombre, correo, duda }) => {
    if (NEXUS_SUPABASE_READY) {
